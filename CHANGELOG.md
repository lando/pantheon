...
* Switched from `mysql`/`mysqldump` to `mariadb`/`mariadb-dump` clients in `lando pull`, `lando push`, and database tooling
...
* Fixed Tika 3 jar download URL that pointed to wrong version directory
* Upgraded MariaDB client to 11.4 LTS on PHP 8.3 and 8.4 images
